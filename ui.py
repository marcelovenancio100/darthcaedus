...
class UI:
    ...
    def show_exp(self, exp):
        exp_surf = self.font.render(str(int(exp)), False, TEXT_COLOR)
        x = self.display_surf.get_size()[0] - 20
        y = self.display_surf.get_size()[1] - 20
        exp_rect = exp_surf.get_rect(bottomright=(x, y))

        pygame.draw.rect(self.display_surf, UI_BG_COLOR, exp_rect.inflate(20, 20))
        self.display_surf.blit(exp_surf, exp_rect)
        pygame.draw.rect(self.display_surf, UI_BORDER_COLOR, exp_rect.inflate(20, 20), 3)
    ...
    def magic_overlay(self, magic_index, has_switched):
        bg_rect = self.show_selection_box(100, 630, has_switched)
